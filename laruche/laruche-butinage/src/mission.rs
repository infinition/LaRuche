...
pub struct ControleMission {
    ...
    pub skills_utilises: BTreeMap<String, String>,
}
...
impl ControleMission {
    ...
    pub fn definir_criteres(&mut self, args: &Value) -> Result<(), String> {
        ...
        let mut criteres: Vec<Critere> =
            serde_json::from_value(raw.clone()).map_err(|e| e.to_string())?;
        if criteres.is_empty() || criteres.len() > 12 {
            return Err("Provide 1 to 12 acceptance criteria".into());
        }
        for c in &mut criteres {
            if c.outil.is_empty()
                || c.description.is_empty()
                || (!c.pointer.is_empty() && !c.pointer.starts_with('/'))
            {
        ...
        if !self.criteres.is_empty() {
            return Err(
                "Acceptance criteria already registered; verify them or report the blocker".into(),
            );
        }
    ...
    pub fn observer(
        ...
        if res.ok
            && (appel.nom == "skill_view"
                || (appel.nom == "tool_call" && appel.args["tool"] == "skill_view"))
        {
            let args = if appel.nom == "tool_call" {
                &appel.args["args"]
            } else {
                &appel.args
            };
            if let Some(name) = args["name"].as_str() {
                let fingerprint =
                    uuid::Uuid::new_v5(&uuid::Uuid::NAMESPACE_OID, res.sortie.as_bytes())
                        .to_string();
                self.skills_utilises.insert(name.into(), fingerprint);
            }
        }
        let action = appel
            .args
        ...
        let nouveau = res.ok
            && !self.preuves.iter().any(|p| {
                p.ok && p.outil == appel.nom && p.action == action && p.empreinte == empreinte
            });
        self.erreurs_consecutives = if res.ok {
            0
        } else {
        ...
        if let Some(v) = &parsed {
            for c in &mut self.criteres {
                if c.outil == appel.nom && (c.action.is_none() || c.action == action) {
                    ...
                    c.preuve = if res.ok && v.pointer(&c.pointer) == Some(&c.equals) {
                        Some(appel.id.clone())
                    } else {
                        None
                    };
        ...
        if res.incertain && !lecture && !self.executions.values().any(|e| e.appel.id == appel.id) {
            self.executions.insert(
                appel.id.clone(),
                Execution {
                    appel: appel.clone(),
                    lecture,
                    resultat: Some(res.clone()),
                },
            );
        }
        self.preuves.push(Preuve {
            appel_id: appel.id.clone(),
            outil: appel.nom.clone(),
            action,
            empreinte,
            ok: res.ok,
            passe,
        });
        if self.preuves.len() > 128 {
            self.preuves.remove(0);
        }
        ...
        let progres =
            nouveau && (self.app_active.is_none() || (appel.nom == "app_call" && !lecture));
        if progres {
            self.derniere_progression = passe;
        }
        progres
    }
    ...
    pub fn obstacle_fin(&self, plan_ouvert: bool, texte: &str) -> Option<String> {
        if self
            .executions
            .values()
            .any(|e| !e.lecture && e.resultat.as_ref().map(|r| r.incertain).unwrap_or(true))
        {
    ...
    pub fn rendu(&self, objectif: &str, plan: &crate::Itineraire) -> String {
        let etapes: Vec<_> = plan
            .etapes
            .iter()
            .map(|e| format!("[{}] {}", e.statut.cle(), e.titre))
            .collect();
        let criteres: Vec<_> = self
            .criteres
            .iter()
            .map(|c| {
                format!(
...
mod tests {
    ...
    #[test]
    fn une_lecture_ne_resout_pas_un_effet_incertain() {
        let mut m = ControleMission::default();
        ...
        let obstacle = m
            .obstacle_fin(false, "voila")
            .expect("an unknown outcome must block");
        assert!(obstacle.contains("unknown outcome"), "got: {obstacle}");
        // A read without explicit evidence cannot reconcile an unknown mutation.
        m.observer(&ecrire, &ResultatOutil::ok("a: 12 bytes"), true, 2);
        assert!(m.obstacle_fin(false, "voila").is_some());
    }
