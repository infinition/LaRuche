...
LaRuche.Console = (function(){
  ...
  function renderIfActive() {
    ...
    if(!el || !document.getElementById('page-console').classList.contains('active')) return;
    render();
  }
...
LaRuche.WS = (function(){
  ...
  function scheduleReconnect() {
    ...
    if(reconnectAttempts > 20) {
      document.getElementById('statusDot').className = 'status-dot';
      document.getElementById('statusText').textContent = 'Serveur indisponible';
      document.getElementById('reconnectBanner').classList.remove('visible');
      return;
    }
    var delay = Math.min(2000*Math.pow(1.5, reconnectAttempts-1), 30000);
    document.getElementById('statusDot').className = 'status-dot reconnecting';
    document.getElementById('statusText').textContent = 'Reconnexion ('+reconnectAttempts+')...';
...
LaRuche.Chat = (function(){
  ...
  function handleEvent(data) {
    switch(data.type) {
      case 'session': sessionId = data.session_id; loadSessions(); break;
      case 'token':
      ...
      case 'thought':
        setFeedLive('thinking');
        addActivity('thinking',thoughtLabel(data.phase||'',data.kind||''),data.text,false);
        break;
      case 'tool_call':
        closeAssistantSegmentForTool();
        setFeedLive('tool');
        var toolLabel=toolActivityLabel(data.name,data.args);
        updateActiveFeedTitle(toolLabel);
  ...
  function closeAssistantSegmentForTool() {
    ...
    if(hasVisibleText) {
      ...
      label.textContent='Étape intermédiaire · avant outil';
      var wrapper=currentAssistantRow.querySelector('.message-wrapper');
      if(wrapper) wrapper.insertBefore(label,currentAssistantMsg);
    } else {
  ...
  function finalizeMessage(el, fallbackText) {
    ...
    var ttsBtn = document.createElement('button'); ttsBtn.className='msg-action-btn'; ttsBtn.innerHTML='&#x1F50A; Lire';
    ttsBtn.onclick=function(){LaRuche.Voice.speakText(text,ttsBtn);}; actions.appendChild(ttsBtn);
    var copyBtn2 = document.createElement('button'); copyBtn2.className='msg-action-btn'; copyBtn2.innerHTML='&#x1F4CB; Copier';
    copyBtn2.onclick=function(){navigator.clipboard.writeText(text).then(function(){copyBtn2.classList.add('copied');copyBtn2.innerHTML='&#x2713; Copie';setTimeout(function(){copyBtn2.classList.remove('copied');copyBtn2.innerHTML='&#x1F4CB; Copier';},2000);});}; actions.appendChild(copyBtn2);
  ...
  function scrollToBottom() { requestAnimationFrame(function(){ var c=document.getElementById('chatContainer'); c.scrollTop=c.scrollHeight; }); }
  ...
  var _planMissions=[], _currentPlanMission=-1;
  ...
  function resetFeed(){
    ...
    _planMissions=[]; _currentPlanMission=-1;
    updateFeedJump();
  }
  function feedAtBottom(af){ return af.scrollHeight-af.scrollTop-af.clientHeight<44; }
  ...
  function followFeed(af,follow){
    if(follow){af.scrollTop=af.scrollHeight;_feedUnread=0;}else{_feedUnread++;}
    updateFeedJump();
  }
  function setFeedLive(mode){
    ...
    if(!_feedTimer)_feedTimer=setInterval(function(){
      ...
      text.textContent=(status.classList.contains('tool')?'Exécution en cours':'Réflexion en cours')+' · '+seconds+' s';
    },1000);
  }
  ...
  function newFeedStep(label){
    ensureFeedStyle();
    var af=document.getElementById('activityFeed');if(!af)return;
    var follow=feedAtBottom(af);_feedStepN++;
    if(_feedPrevCard){_feedPrevCard.classList.remove('expanded','running');var previousState=_feedPrevCard.querySelector('.feed-step-state');if(previousState)previousState.textContent='TERMINÉ';}
    var card=document.createElement('section');card.className='feed-step expanded running';
    var h=document.createElement('div');h.className='feed-step-head';
    h.innerHTML='<span class="feed-step-caret">▾</span><span class="feed-step-title">'+LaRuche.Utils.esc(label||_feedTurnTitle||('Cycle '+_feedStepN))+'</span><span class="feed-step-state">EN COURS</span><span class="feed-step-meta"></span>';
    h.onclick=function(){card.classList.toggle('expanded');};
    var b=document.createElement('div');b.className='feed-step-body';card.appendChild(h);card.appendChild(b);af.appendChild(card);
    _feedBody=b;_feedPrevCard=card;pruneFeedHistory(af);followFeed(af,follow);
  }
  ...
  function addActivity(type,label,body,collapsible,options){
    ensureFeedStyle();var af=document.getElementById('activityFeed');if(!af)return null;
    var follow=feedAtBottom(af);options=options||{};
    if(!_feedBody)newFeedStep(options.stepTitle||_feedTurnTitle);
    var iconMap={'thinking':'◌','tool-call':'⌘','tool-ok':'✓','tool-err':'!','status':'•'};
    var item=document.createElement('article');item.className='act '+type+(type.indexOf('tool-')===0?' act-major':' act-noise')+(collapsible?' collapsible':'')+(options.live?' live':'')+(options.terminal?' terminal':'');
    ...
    line.innerHTML='<span class="act-ico '+type+'">'+(iconMap[type]||'•')+'</span><span class="act-txt">'+LaRuche.Utils.esc(label)+'</span><span class="act-time">'+new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit',second:'2-digit'})+'</span>';
    item.appendChild(line);
    if(body!==undefined&&body!==null&&body!==''){var bodyEl=document.createElement('pre');bodyEl.className='act-body';if(options.terminal){item.dataset.command=options.command||body;setTerminalContent(bodyEl,item.dataset.command,'');}else bodyEl.textContent=body;item.appendChild(bodyEl);if(collapsible)line.onclick=function(){item.classList.toggle('open');};}
    if(options.live)_feedPendingTools.push(item);
    (_feedBody||af).appendChild(item);updateFeedMeta();followFeed(af,follow);
  ...
  function finishToolActivity(name,result,success,elapsed){
    ...
    if(!success)item.classList.add('open');_feedPendingTools=_feedPendingTools.filter(function(candidate){return candidate!==item;});updateFeedMeta();return item;
  }
  ...
  function renderPlanMissions(){
    var ps=document.getElementById('planSection'); if(!ps)return;
    ps.innerHTML='<div class="plan-title">Missions</div>';
    _planMissions.forEach(function(mission,index){
      var doneCount=mission.items.filter(planItemDone).length;
      var box=document.createElement('details'); box.className='plan-mission';
      if(index===_currentPlanMission) box.open=true;
      var summary=document.createElement('summary'); summary.textContent=mission.title;
      var meta=document.createElement('span'); meta.className='plan-mission-meta'; meta.textContent=mission.items.length?(doneCount+'/'+mission.items.length+' terminée(s)'):'en attente du plan'; summary.appendChild(meta); box.appendChild(summary);
      mission.items.forEach(function(item){
        var row=document.createElement('div'); row.className='plan-item';
        var dot=document.createElement('span'); dot.className='plan-dot '+planDotClass(item);
        var text=document.createElement('span'); text.className='plan-text'+(planItemDone(item)?' done':''); text.textContent=item.task;
        row.appendChild(dot); row.appendChild(text); box.appendChild(row);
      });
      ps.appendChild(box);
    });
  }
  function updatePlan(items) {
    ...
    var nextTitle=active&&active.task ? active.task : mission.title;
    if(nextTitle && mission.activeTask!==nextTitle){
      mission.activeTask=nextTitle;
      _feedTurnTitle=nextTitle;
      if(!_feedBody) newFeedStep(nextTitle);
      else if(_feedPrevCard && _feedPrevCard.querySelector('.feed-step-title') && _feedPrevCard.querySelector('.feed-step-title').textContent!==nextTitle) newFeedStep(nextTitle);
      else updateActiveFeedTitle(nextTitle);
    }
    renderPlanMissions();
  }
  ...
  function loadSessions() {
    fetch('/api/sessions').then(function(r){return r.json();}).then(function(sessions){
      ...
      sessions.forEach(function(s){
        ...
        div.innerHTML='<span class="session-title">'+LaRuche.Utils.esc(s.title||'Sans titre')+'</span><span class="session-delete" onclick="event.stopPropagation();LaRuche.Chat.deleteSession(\''+s.id+'\')">&times;</span>';
  ...
  function newSession() {
    ...
    welcome.innerHTML='<div class="honeycomb-loader hc-lg" style="margin:0 auto 16px"><div class="hexagon hx1"></div><div class="hexagon hx2"></div><div class="hexagon hx3"></div><div class="hexagon hx4"></div><div class="hexagon hx5"></div><div class="hexagon hx6"></div><div class="hexagon hx7"></div></div><h2>L\x27Essaim</h2><p>Agent IA propulse par LaRuche.<br>Posez votre question ou donnez une instruction.</p><div class="suggested-prompts"><div class="suggested-prompt" onclick="LaRuche.Chat.useSuggestion(this)">Explique-moi l\'architecture du projet</div><div class="suggested-prompt" onclick="LaRuche.Chat.useSuggestion(this)">Quels fichiers ont ete modifies recemment ?</div><div class="suggested-prompt" onclick="LaRuche.Chat.useSuggestion(this)">Analyse les erreurs dans les logs</div><div class="suggested-prompt" onclick="LaRuche.Chat.useSuggestion(this)">Cree un plan d\'action pour optimiser les performances</div></div>';
    ...
    fetch('/api/sessions/'+id+'/messages').then(function(r){return r.ok?r.json():null;}).then(function(data){
      ...
      data.messages.forEach(function(msg){
        ...
        if(msg.role==='user'){startPlanMission(msg.text);addMessage('user',msg.text, msg.attachments); _feedBody=null; /* nouveau tour → nouvelle étape */ }
          ...
          addActivity('thinking',thoughtLabel(msg.phase||'',msg.kind||''),msg.text||'',false,{stepTitle:_feedTurnTitle});
        }
          ...
          var copyBtn=document.createElement('button');copyBtn.className='msg-action-btn';copyBtn.innerHTML='&#x1F4CB; Copier';
          copyBtn.onclick=function(){navigator.clipboard.writeText(restored.text).then(function(){copyBtn.classList.add('copied');copyBtn.innerHTML='&#x2713; Copie';setTimeout(function(){copyBtn.classList.remove('copied');copyBtn.innerHTML='&#x1F4CB; Copier';},2000);});};actions.appendChild(copyBtn);
          el.appendChild(actions);
        }
  ...
  return {
    ...
    browseCwd:browseCwd, loadCwd:loadCwd, toggleNoThink:toggleNoThink
  };
})();
...
LaRuche.Voice = (function(){
  ...
  async function speakText(text, btn) {
    if(btn && btn.classList.contains('playing')){stopAllTts();btn.classList.remove('playing');btn.innerHTML='&#x1F50A; Lire';return;}
    ...
    try {
      ...
      currentTtsAudio.onended=function(){if(btn){btn.classList.remove('playing');btn.innerHTML='&#x1F50A; Lire';}currentTtsAudio=null;};
      currentTtsAudio.onerror=function(){if(btn){btn.classList.remove('playing');btn.innerHTML='&#x1F50A; Lire';}currentTtsAudio=null;};
  ...
  function speakBrowser(text, btn) {
    if('speechSynthesis' in window){
      ...
      utter.onend=function(){if(btn){btn.classList.remove('playing');btn.innerHTML='&#x1F50A; Lire';}currentTtsUtterance=null;};
      utter.onerror=function(){if(btn){btn.classList.remove('playing');btn.innerHTML='&#x1F50A; Lire';}currentTtsUtterance=null;};
      speechSynthesis.speak(utter);
    } else {
      if(btn){btn.classList.remove('playing');btn.innerHTML='&#x1F50A; Lire';}
    }
  }
...
LaRuche.Dashboard = (function(){
  ...
  async function fetchContextStats(){
    try{
      var r=await fetch(LaRuche.API.base+'/api/context/stats');
      if(!r.ok)return;
      var d=await r.json();
      var el = document.getElementById('headerContextUsage');
      if(el) {
        var pct = Math.round((d.ratio||0)*100);
        el.textContent = 'CTX: ' + pct + '%';
        if(pct > 90) el.style.color = 'var(--red)';
        else if(pct > 75) el.style.color = 'var(--amber)';
  ...
  function startPolling(){
    ...
    pollTimers.push(setInterval(fetchContextStats,5000));
  ...
  function init(){
    ...
    inferInput.addEventListener('input',function(){this.style.height='auto';this.style.height=Math.min(this.scrollHeight,120)+'px';});
    document.getElementById('infer-btn').addEventListener('click',runInference);
    inferInput.addEventListener('keydown',function(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();runInference();}});
    // Popover delegation
  ...
  return { init:init, openBlueprintForm:openBlueprintForm, instanciateBlueprint:instanciateBlueprint, enter:enter, leave:leave, toggleNodeExpand:toggleNodeExpand, useMeshModel:useMeshModel };
})();

/* ── Sessions Page Module ─────────────────────────────────────── */
LaRuche.SessionsPage = (function(){
  var allSessions = [];

  function init() {}

  function enter() { loadSessions(); }
  function leave() {}

  function loadSessions() {
    fetch('/api/sessions').then(function(r){return r.json();}).then(function(sessions){
      allSessions = sessions;
      allSessions.sort(function(a,b){return new Date(b.updated_at)-new Date(a.updated_at);});
  ...
  function renderSessions(sessions) {
    ...
    tbody.innerHTML = '';
    if(!sessions.length){tbody.innerHTML='<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:20px">Aucune session</td></tr>';return;}
    sessions.forEach(function(s){
      var tr = document.createElement('tr');
      var updated = s.updated_at ? new Date(s.updated_at).toLocaleString('fr-FR',{dateStyle:'short',timeStyle:'short'}) : '-';
      tr.innerHTML =
        '<td>'+LaRuche.Utils.esc(s.title||'Sans titre')+'</td>'+
        '<td style="text-align:center">'+(s.messages||'-')+'</td>'+
        '<td>'+updated+'</td>'+
        '<td><div class="s-actions">'+
          '<button class="s-btn s-btn-open" data-id="'+s.id+'">Ouvrir</button>'+
          '<button class="s-btn s-btn-export" data-id="'+s.id+'">Export</button>'+
          '<button class="s-btn s-btn-del" data-id="'+s.id+'">Supprimer</button>'+
        '</div></td>';
      tr.querySelector('.s-btn-open').onclick = function(){ LaRuche.Router.go('chat'); setTimeout(function(){LaRuche.Chat.switchSession(s.id);},100); };
      tr.querySelector('.s-btn-export').onclick = function(){ fetch('/api/sessions/'+s.id+'/export').then(function(r){return r.text();}).then(function(md){var a=document.createElement('a');a.href=URL.createObjectURL(new Blob([md]));a.download='session-'+s.id+'.md';a.click();}); };
      tr.querySelector('.s-btn-del').onclick = function(){ fetch('/api/sessions/'+s.id,{method:'DELETE'}).then(function(){loadSessions();LaRuche.Chat.loadSessions();}); };
  ...
  function search(q) {
    ...
    fetch('/api/sessions/search?q='+encodeURIComponent(q)).then(function(r){return r.json();}).then(function(results){
      renderSessions(results);
    }).catch(function(){
      // Fallback: client-side filter
      var filtered = allSessions.filter(function(s){return (s.title||'').toLowerCase().includes(q);});
  ...
  return { init:init, openBlueprintForm:openBlueprintForm, instanciateBlueprint:instanciateBlueprint, enter:enter, leave:leave, search:search };
})();
...
LaRuche.Memory = (function(){
  ...
  function init() {
    ...
    if(input) input.addEventListener('input', function(){ queueSuggest(input.value); });
    var writeNode = document.getElementById('memoryWriteNode');
    if(writeNode) writeNode.addEventListener('input', function(){ queueSuggest(writeNode.value); });
    var dreamBtn = document.getElementById('memoryDreamBtn');
    if(dreamBtn) dreamBtn.addEventListener('click', triggerDream);
  }

  function addThoughtMessage(phase, kind, text) {
    var container = document.getElementById('chatContainer');
    var row = document.createElement('div');
    ...
    msg.style.border = '1px solid var(--amber-dim)';
    msg.innerHTML = '<details open style="outline:none;"><summary class="tool-msg-head" style="cursor:pointer; display:flex; justify-content:space-between; list-style:none;"><span style="color:var(--amber)">[Pensée - '+LaRuche.Utils.esc(phase)+']</span><span>'+LaRuche.Utils.esc(kind)+'</span></summary>'+
      '<div style="color:var(--text-dim);font-style:italic; padding-top:8px; white-space:pre-wrap;">'+LaRuche.Utils.esc((text||''))+'</div></details>';
    wrapper.appendChild(msg);
    row.appendChild(wrapper);
    container.appendChild(row);
    scrollToBottom();
  ...
  function addToolMessage(name, result, success, elapsedMs) {
    var container = document.getElementById('chatContainer');
    var row = document.createElement('div');
    row.className = 'message-row tool-row';
    var wrapper = document.createElement('div');
    wrapper.className = 'message-wrapper';
    var msg = document.createElement('div');
    msg.className = 'message tool'+(success?'':' err');
    var elapsed = elapsedMs != null ? ' · '+elapsedMs+' ms' : '';
    msg.innerHTML = '<div class="tool-msg-head"><span class="tool-msg-status '+(success?'':'err')+'">'+(success?'OK':'ERR')+'</span><span>'+LaRuche.Utils.esc(name)+elapsed+'</span></div>'+
      '<div>'+LaRuche.Utils.esc((result||'').substring(0,2000))+'</div>';
    wrapper.appendChild(msg);
    row.appendChild(wrapper);
    container.appendChild(row);
    scrollToBottom();
  ...
  function mergeNodes(incoming) {
    var byId = {};
    nodes.forEach(function(n){ byId[n.id || n.label] = n; });
    (incoming || []).forEach(function(n){
      var id = n.id || n.node_id || n.label || 'node';
      byId[id] = { id:id, label:n.label || id, one_liner:n.one_liner || n.summary || '' };
    });
    nodes = Object.keys(byId).map(function(k){ return byId[k]; });
  }

  function loadTree() {
    fetch('/api/memory/tree').then(function(r){return r.json();}).then(function(data){
      loaded = true;
      mergeNodes(data.nodes || []);
      document.getElementById('memoryHealth').textContent = data.health ? 'ok' : 'down';
      document.getElementById('memoryHealth').style.color = data.health ? 'var(--green)' : 'var(--red)';
      document.getElementById('memoryAudit').innerHTML = '<pre>'+LaRuche.Utils.esc(JSON.stringify(data.audit || {}, null, 2))+'</pre>';
      renderAll();
      refreshSuggestions('');
      loadProposed();
    }).catch(function(e){
      document.getElementById('memoryNodeList').innerHTML = '<div class="memory-empty">Memoire indisponible</div>';
      LaRuche.Toast.show('Memoire indisponible: '+e, 'err');
    });
  }

  function queueSuggest(q) {
    clearTimeout(suggestTimer);
    suggestTimer = setTimeout(function(){ refreshSuggestions(q || ''); }, 180);
  }

  function refreshSuggestions(q) {
    fetch('/api/memory/suggest?q='+encodeURIComponent(q || '')+'&limit=20').then(function(r){return r.json();}).then(function(data){
      var raw = data.result || data;
      mergeNodes(raw.nodes || []);
      var dl = document.getElementById('memoryNodeSuggestions');
      if(dl) {
        dl.innerHTML = (raw.nodes || []).map(function(n){
          var id = n.id || n.node_id || n.label;
          return '<option value="'+LaRuche.Utils.esc(id)+'">'+LaRuche.Utils.esc((n.one_liner || n.label || '') + (n.item_count != null ? ' · '+n.item_count : ''))+'</option>';
        }).join('');
      }
      renderNodeList();
      renderSvg();
  ...
  function loadProposed() {
    fetch('/api/memory/proposed?limit=20').then(function(r){return r.json();}).then(function(data){
      var raw = data.result || {};
      var proposed = raw.items || [];
      var audit = document.getElementById('memoryAudit');
      if(!audit) return;
      if(!proposed.length) return;
      audit.innerHTML = '<div class="memory-empty" style="text-align:left">Propositions en attente</div>' + proposed.map(function(it){
        return '<div class="memory-item" data-item="'+LaRuche.Utils.esc(it.id)+'">'+
          '<div class="memory-item-node">'+LaRuche.Utils.esc(it.node_id || '')+' · '+LaRuche.Utils.esc(it.id || '')+'</div>'+
          LaRuche.Utils.esc(it.content || '')+
          '<div class="memory-item-actions">'+
            '<button class="memory-mini-btn" data-review="accept" data-id="'+LaRuche.Utils.esc(it.id)+'">Accepter</button>'+
            '<button class="memory-mini-btn danger" data-review="reject" data-id="'+LaRuche.Utils.esc(it.id)+'">Rejeter</button>'+
          '</div></div>';
      }).join('');
      audit.querySelectorAll('[data-review]').forEach(function(btn){
        btn.onclick = function(){ reviewItem(btn.dataset.id, btn.dataset.review); };
      });
    }).catch(function(){});
  }
  ...
  function search() {
    ...
    if(!q) return;
    fetch('/api/memory/search?q='+encodeURIComponent(q)+'&limit=12').then(function(r){return r.json();}).then(function(data){
      if(data.error) { LaRuche.Toast.show(data.error, 'err'); return; }
      var raw = data.raw || {};
      items = raw.items || raw.evidence || [];
      active = {};
      (raw.nodes || []).forEach(function(n){ var id = n.id || n.node_id || n.label; if(id) active[id] = 1; });
      mergeNodes(raw.nodes || []);
      document.getElementById('memorySummary').textContent = items.length+' souvenir(s) pour "'+q+'"';
      renderAll();
    }).catch(function(e){ LaRuche.Toast.show('Recherche memoire: '+e, 'err'); });
  }

  function loadNode(id) {
    id = String(id || '').trim();
    if(!id) return;
    fetch('/api/memory/node/'+encodeURIComponent(id)).then(function(r){return r.json();}).then(function(data){
      if(data.status === 'error' || data.error) { LaRuche.Toast.show(data.error || 'Noeud indisponible', 'err'); return; }
      var node = data.node || data;
      var nodeId = node.id || node.node_id || id;
      var children = node.children || [];
      items = node.items || [];
      active = {};
      active[nodeId] = 1;
      mergeNodes([node].concat(children));
      document.getElementById('memorySearchInput').value = nodeId;
      document.getElementById('memorySummary').textContent = nodeId+' · '+children.length+' enfant(s) · '+items.length+' item(s)';
      renderAll();
    }).catch(function(e){ LaRuche.Toast.show('Lecture noeud memoire: '+e, 'err'); });
  }

  function write() {
    var node = (document.getElementById('memoryWriteNode').value || '').trim();
    var content = (document.getElementById('memoryWriteContent').value || '').trim();
    if(!node || !content) { LaRuche.Toast.show('node_id et contenu requis', 'warn'); return; }
    fetch('/api/memory/write', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({node_id:node, content:content, source:'ui-memory'})
    }).then(function(r){return r.json();}).then(function(data){
      if(data.status === 'error' || data.error) { LaRuche.Toast.show(data.error || 'Erreur memoire', 'err'); return; }
      document.getElementById('memoryWriteContent').value = '';
      active[node] = 1;
      mergeNodes([{id:node, label:node, one_liner:'Ajoute depuis UI'}]);
      document.getElementById('memorySearchInput').value = content;
      LaRuche.Toast.show('Souvenir memorise', 'ok');
      loadNode(node);
      loadTree();
    }).catch(function(e){ LaRuche.Toast.show('Ecriture memoire: '+e, 'err'); });
  }

  function editItem(id) {
    var current = items.find(function(it){ return String(it.id || '') === String(id || ''); });
    var next = window.prompt('Modifier le souvenir', current ? (current.content || current.text || '') : '');
    if(next == null) return;
    next = next.trim();
    if(!next) { LaRuche.Toast.show('Contenu requis', 'warn'); return; }
    postJson('/api/memory/update', {item_id:id, content:next}).then(function(data){
      if(data.status === 'error' || data.error) { LaRuche.Toast.show(data.error || 'Update memoire impossible', 'err'); return; }
      LaRuche.Toast.show('Souvenir mis a jour', 'ok');
      if(current) { current.content = next; }
      renderItems();
    }).catch(function(e){ LaRuche.Toast.show('Update memoire: '+e, 'err'); });
  }

  function deleteItem(id) {
    if(!window.confirm('Supprimer ce souvenir ?')) return;
    postJson('/api/memory/delete', {item_id:id, reason:'ui-memory'}).then(function(data){
      if(data.status === 'error' || data.error) { LaRuche.Toast.show(data.error || 'Suppression impossible', 'err'); return; }
      items = items.filter(function(it){ return String(it.id || '') !== String(id || ''); });
      LaRuche.Toast.show('Souvenir supprime', 'ok');
      renderItems();
      loadTree();
    }).catch(function(e){ LaRuche.Toast.show('Suppression memoire: '+e, 'err'); });
  }

  function moveItem(id) {
    var current = items.find(function(it){ return String(it.id || '') === String(id || ''); });
    var nextNode = window.prompt('Deplacer vers node_id', current ? (current.node_id || '') : '');
    if(nextNode == null) return;
    nextNode = nextNode.trim();
    if(!nextNode) { LaRuche.Toast.show('node_id requis', 'warn'); return; }
    postJson('/api/memory/move', {item_id:id, node_id:nextNode}).then(function(data){
      if(data.status === 'error' || data.error) { LaRuche.Toast.show(data.error || 'Deplacement impossible', 'err'); return; }
      if(current) current.node_id = nextNode;
      mergeNodes([{id:nextNode, label:nextNode}]);
      LaRuche.Toast.show('Souvenir deplace', 'ok');
      renderAll();
    }).catch(function(e){ LaRuche.Toast.show('Deplacement memoire: '+e, 'err'); });
  }

  function reviewItem(id, action) {
    postJson('/api/memory/review', {item_id:id, action:action, reason:'ui-memory'}).then(function(data){
      if(data.status === 'error' || data.error) { LaRuche.Toast.show(data.error || 'Revue impossible', 'err'); return; }
      LaRuche.Toast.show(action === 'accept' ? 'Proposition acceptee' : 'Proposition rejetee', 'ok');
      loadProposed();
      loadTree();
    }).catch(function(e){ LaRuche.Toast.show('Revue memoire: '+e, 'err'); });
  }

  function renderAll() { renderNodeList(); renderItems(); renderSvg(); }

  function buildTree() {
    var root = { children:{} };
    nodes.forEach(function(n){
      var id = n.id || n.label; if(!id) return;
      var cur = root, path = '';
      String(id).split('.').forEach(function(p){
        path = path ? path+'.'+p : p;
  ...
  function treeLevelHtml(node, depth) {
    var html = '';
    Object.keys(node.children).sort().forEach(function(k){
      var c = node.children[k];
      var kids = Object.keys(c.children).length;
      var open = depth === 0 ? (expanded[c.id] !== false) : !!expanded[c.id];
      var caret = kids ? (open ? '▾' : '▸') : '·';
      html += '<div class="memory-tree-row'+(active[c.id]?' active':'')+'" style="padding-left:'+(6+depth*14)+'px" data-node="'+LaRuche.Utils.esc(c.id)+'">'+
        '<span class="memory-tree-caret" data-toggle="'+LaRuche.Utils.esc(c.id)+'">'+caret+'</span>'+
        '<span class="memory-tree-label">'+LaRuche.Utils.esc(c.seg)+'</span>'+
        (kids ? '<span class="memory-tree-count">'+kids+'</span>' : '')+
      '</div>';
      if(kids && open) html += treeLevelHtml(c, depth+1);
    });
    return html;
  }

  function renderNodeList() {
    var el = document.getElementById('memoryNodeList');
    if(!nodes.length) { el.innerHTML = '<div class="memory-empty">Aucun noeud</div>'; return; }
    el.innerHTML = treeLevelHtml(buildTree(), 0);
    el.querySelectorAll('.memory-tree-caret').forEach(function(c){
      c.onclick = function(e){
        e.stopPropagation();
        var id = c.dataset.toggle;
        var cur = (typeof expanded[id] === 'undefined') ? (id.indexOf('.') < 0) : expanded[id];
        expanded[id] = !cur;
        renderNodeList();
      };
    });
    el.querySelectorAll('.memory-tree-row').forEach(function(row){
      row.onclick = function(){ loadNode(row.dataset.node); };
    });
  }

  // Sous-ensemble FOCALISÉ pour la carte : nœud actif + parent + enfants + voisins (pas tout).
  function focusNodes() {
    var byId = {}; nodes.forEach(function(n){ var id=n.id||n.label; if(id) byId[id]=n; });
    var ids = Object.keys(byId);
    var focusId = Object.keys(active)[0];
    if(!focusId) {
      var roots = ids.filter(function(id){ return id.indexOf('.') < 0; }).map(function(id){ return byId[id]; });
      return roots.length ? roots : ids.slice(0, 12).map(function(id){ return byId[id]; });
    }
    var set = {}; set[focusId] = 1;
    var parent = focusId.indexOf('.') >= 0 ? focusId.slice(0, focusId.lastIndexOf('.')) : null;
    if(parent && byId[parent]) set[parent] = 1;
    ids.forEach(function(id){
      var p = id.indexOf('.') >= 0 ? id.slice(0, id.lastIndexOf('.')) : null;
      if(p === focusId) set[id] = 1;          // enfants
      if(parent && p === parent) set[id] = 1; // voisins
    });
    return Object.keys(set).slice(0, 14).map(function(id){ return byId[id] || {id:id, label:id}; });
  }

  function renderItems() {
    var el = document.getElementById('memoryResults');
    document.getElementById('memoryItemCount').textContent = String(items.length || 0);
    if(!items.length) { el.innerHTML = '<div class="memory-empty">Aucun item trouve.</div>'; return; }
    el.innerHTML = items.map(function(it){
      var node = it.node_id || it.node || it.id || '';
      var id = it.id || '';
      var content = it.content || it.text || JSON.stringify(it);
      return '<div class="memory-item">'+
        (node ? '<div class="memory-item-node">'+LaRuche.Utils.esc(node)+(id ? ' · '+LaRuche.Utils.esc(id) : '')+'</div>' : '')+
        LaRuche.Utils.esc(content)+
        (id ? '<div class="memory-item-actions">'+
          '<button class="memory-mini-btn" data-action="edit" data-id="'+LaRuche.Utils.esc(id)+'">Editer</button>'+
          '<button class="memory-mini-btn" data-action="move" data-id="'+LaRuche.Utils.esc(id)+'">Deplacer</button>'+
          '<button class="memory-mini-btn danger" data-action="delete" data-id="'+LaRuche.Utils.esc(id)+'">Supprimer</button>'+
        '</div>' : '')+
      '</div>';
    }).join('');
    el.querySelectorAll('[data-action]').forEach(function(btn){
      btn.onclick = function(){
        ...
        if(btn.dataset.action === 'move') moveItem(btn.dataset.id);
  ...
  function renderSvg() {
    var svg = document.getElementById('memorySvg');
    if(!svg) return;
    var shown = focusNodes();
    if(!shown.length) shown = [{id:'memoire',label:'memoire',one_liner:''}];
    var cols = Math.max(3, Math.ceil(Math.sqrt(shown.length)));
    var gapX = 150, gapY = 126, r = 54;
    var startX = 120, startY = 95;
    var html = '<defs><filter id="memGlow"><feGaussianBlur stdDeviation="4" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>';
    shown.forEach(function(n, idx){
      var row = Math.floor(idx / cols);
      var col = idx % cols;
      var x = startX + col * gapX + (row % 2 ? gapX / 2 : 0);
      var y = startY + row * gapY;
      var id = n.id || n.label;
      var isActive = !!active[id];
      var fill = isActive ? 'rgba(245,158,11,.42)' : 'rgba(24,24,27,.88)';
      var stroke = isActive ? 'var(--amber)' : 'var(--border-light)';
      var filter = isActive ? ' filter="url(#memGlow)"' : '';
      html += '<g class="memory-hex" data-node="'+LaRuche.Utils.esc(id)+'" style="cursor:pointer">'+
        '<polygon points="'+hexPoints(x,y,r)+'" fill="'+fill+'" stroke="'+stroke+'" stroke-width="'+(isActive?3:1.5)+'"'+filter+'></polygon>'+
        '<text x="'+x+'" y="'+(y-4)+'" text-anchor="middle" fill="var(--text)" font-size="13" font-family="Segoe UI, sans-serif" font-weight="700">'+LaRuche.Utils.esc(shortLabel(id))+'</text>'+
        '<text x="'+x+'" y="'+(y+17)+'" text-anchor="middle" fill="var(--text-muted)" font-size="10" font-family="Cascadia Code, monospace">'+(isActive?'active':'latent')+'</text>'+
      '</g>';
    });
    svg.innerHTML = html;
    svg.querySelectorAll('.memory-hex').forEach(function(g){
      g.addEventListener('click', function(){ loadNode(g.dataset.node); });
    });
  }

  function triggerDream() {
    fetch(LaRuche.API.base+'/api/memory/dream', {method:'POST'})
      .then(function(r){
        if(r.ok) LaRuche.Toast.show('Consolidation declenchee en arriere-plan', 'ok');
        else LaRuche.Toast.show('Erreur de consolidation', 'err');
      })
      .catch(function(e){ LaRuche.Toast.show('Erreur: '+e, 'err'); });
  }

  return {
    init:init, enter:enter, leave:leave,
    search:search, write:write, loadNode:loadNode,
    editItem:editItem, deleteItem:deleteItem, moveItem:moveItem, reviewItem:reviewItem,
    triggerDream:triggerDream
  };
})();
...
LaRuche.Settings = (function(){
  ...
  function loadTab(tab) {
    ...
    switch(tab) {
      ...
      case 'sessions': loadSessionsTab(el); break;
  ...
  async function loadGeneral(el) {
    var doc={},voice={},provCfg={},ctxStats={},ctxCfg={};
    try{doc=await fetch('/api/doctor').then(function(r){return r.json();});}catch(e){}
    try{voice=await fetch('/api/voice/status').then(function(r){return r.json();});}catch(e){}
    try{provCfg=await fetch('/api/config/provider').then(function(r){return r.json();});}catch(e){}
    try{ctxStats=await fetch('/api/context/stats').then(function(r){return r.json();});}catch(e){}
    try{ctxCfg=await fetch('/api/config/compaction').then(function(r){return r.json();});}catch(e){}
    var pct = Math.round((ctxStats.ratio||0)*100);
    el.innerHTML = '<div class="settings-grid">'+
      '<div class="settings-card"><div class="settings-card-title">Contexte</div>'+
      '<div class="settings-row" style="flex-direction:column;align-items:stretch;gap:4px;">'+
      '<div style="display:flex;justify-content:space-between;font-size:11px;color:var(--text-dim);"><span>Usage ('+(ctxStats.messages||0)+'/'+(ctxStats.max||0)+')</span><span>'+pct+'%</span></div>'+
      '<div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden;margin-bottom:8px;"><div style="height:100%;background:'+(pct>90?'var(--red)':pct>75?'var(--amber)':'var(--green)')+';width:'+pct+'%;transition:width 0.3s ease;"></div></div>'+
  ...
  async function loadProviders(el) {
    ...
    // Carte dédiée : connexion ChatGPT Codex via abonnement (OAuth).
    ...
    ids.forEach(function(id) {
      ...
      var isPublic = p.visibility === 'public_proxy';
  ...
  async function loadSessionsTab(el) {
    var sessions=[];try{sessions=await fetch('/api/sessions').then(function(r){return r.json();});}catch(e){}
    if(!sessions.length){el.innerHTML='<div style="text-align:center;color:var(--text-muted);padding:20px">No sessions</div>';return;}
    sessions.sort(function(a,b){return new Date(b.updated_at)-new Date(a.updated_at);});
    el.innerHTML='<table style="width:100%;border-collapse:collapse;font-size:12px"><tr><th style="text-align:left;padding:6px;color:var(--text-dim);border-bottom:1px solid var(--border)">Title</th><th style="padding:6px;color:var(--text-dim);border-bottom:1px solid var(--border)">Msgs</th><th style="padding:6px;color:var(--text-dim);border-bottom:1px solid var(--border)">Actions</th></tr>'+sessions.map(function(s){return '<tr><td style="padding:6px;border-bottom:1px solid rgba(42,42,46,.3)">'+LaRuche.Utils.esc(s.title||'Sans titre')+'</td><td style="padding:6px;text-align:center;border-bottom:1px solid rgba(42,42,46,.3)">'+(s.messages||'-')+'</td><td style="padding:6px;text-align:center;border-bottom:1px solid rgba(42,42,46,.3)"><button onclick="fetch(\'/api/sessions/'+s.id+'/export\').then(function(r){return r.text()}).then(function(md){var a=document.createElement(\'a\');a.href=URL.createObjectURL(new Blob([md]));a.download=\'session.md\';a.click()})" style="background:none;border:1px solid var(--border);color:var(--text-dim);border-radius:4px;padding:2px 8px;cursor:pointer;font-size:10px">Export</button> <button onclick="fetch(\'/api/sessions/'+s.id+'\',{method:\'DELETE\'}).then(function(){LaRuche.Settings.refreshTab()})" style="background:none;border:1px solid var(--red);color:var(--red);border-radius:4px;padding:2px 8px;cursor:pointer;font-size:10px">Del</button></td></tr>';}).join('')+'</table>';
  }
  ...
  async function loadNetwork(el) {
    ...
    }).join('')||'<div style="text-align:center;color:var(--text-muted);padding:20px">No nodes</div>';
  }
  ...
  async function loadWatchers(el) {
    ...
    el.innerHTML='<div style="margin-bottom:12px"><button class="settings-save-btn" onclick="document.getElementById(\'newWatcherForm\').style.display=\'block\'">+ New Watcher</button></div>'+
      ...
      watchers.map(function(w){return '<div class="settings-card"><div class="settings-card-title">'+LaRuche.Utils.esc(w.name)+'</div><div class="settings-row"><span class="settings-label">Type</span><span class="settings-value">'+LaRuche.Utils.esc(w.watcher_type)+'</span></div><div class="settings-row"><span class="settings-label">Target</span><span class="settings-value">'+LaRuche.Utils.esc(w.target)+'</span></div><div class="settings-row"><span class="settings-label">Runs</span><span class="settings-value">'+(w.run_count||0)+'</span></div><button onclick="fetch(\'/api/watchers/'+w.id+'\',{method:\'DELETE\'}).then(function(){LaRuche.Settings.refreshTab()})" style="background:none;border:1px solid var(--red);color:var(--red);border-radius:4px;padding:2px 8px;cursor:pointer;font-size:10px;margin-top:6px">Delete</button></div>';}).join('');
  }
  ...
  function createWatcher() {
    ...
    fetch('/api/watchers',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,watcher_type:type,target:target,condition:cond,prompt:prompt})}).then(function(){loadTab('watchers');LaRuche.Toast.show('Watcher created','ok');});
  }
  ...
  // ── Page Skills (OKF en mémoire, tools.skills.*) ──────────────────
  ...
  async function loadSkills(el){
    ...
    if(!skills.length){ html+='<div style="color:var(--text-dim);padding:20px">Aucun skill. L\'agent peut en créer (memory_write tools.skills.*) ou clique « + Nouveau skill ».</div>'; }
    html+='<div class="settings-grid">';
  ...
  function skillEditor(name, content){
    ...
    ov.innerHTML='<div style="width:680px;max-width:94vw;height:80vh;background:#0d0d10;border:1px solid var(--amber);border-radius:10px;display:flex;flex-direction:column">'+
      ...
      '<textarea id="skEditor" class="form-input" style="flex:1;margin:12px;font-family:var(--mono);font-size:12px;resize:none">'+LaRuche.Utils.esc(content)+'</textarea>'+
  ...
  async function loadKanban(el) {
    el.innerHTML = '<div style="margin-bottom:16px;display:flex;gap:8px;align-items:end">' +
      '<div style="flex:1"><label class="form-label">Titre de la tâche</label><input class="form-input" id="kanban-title" placeholder="Nouvelle tâche..."></div>' +
      '<div style="flex:2"><label class="form-label">Description</label><input class="form-input" id="kanban-desc" placeholder="Détails..."></div>' +
  ...
  async function refreshKanbanCols(){
    ...
    var sig=JSON.stringify(tasks); if(sig===_kanbanLast) return; _kanbanLast=sig;
    var cols=['Triage','Todo','Ready','Running','Blocked','Done','Archived'];
    var html='<div style="display:flex;gap:12px;overflow-x:auto;padding-bottom:10px;min-height:400px">';
    cols.forEach(function(c){
      ...
      colTasks.forEach(function(t){
        html+='<div draggable="true" ondragstart="LaRuche.Settings.kanbanDragStart(event,\''+t.id+'\')" style="background:#2a2a2e;border:1px solid var(--border);border-radius:4px;padding:8px;cursor:grab">';
        html+='<div style="font-size:13px;font-weight:600;color:#fff;margin-bottom:4px">'+LaRuche.Utils.esc(t.title)+'</div>';
        html+='<div style="font-size:11px;color:var(--text-dim);margin-bottom:6px">'+LaRuche.Utils.esc(t.description||'')+'</div>';
        if(t.result) html+='<div style="font-size:10px;color:var(--green);margin-bottom:6px">Résultat: '+LaRuche.Utils.esc((t.result||'').substring(0,60))+'…</div>';
        html+='<div style="display:flex;justify-content:space-between;align-items:center">';
        html+='<span style="font-size:9px;color:var(--text-muted);font-family:var(--mono)">'+t.id.split('-')[0]+'</span>';
...
var m = document.getElementById('kanban-model')?document.getElementById('kanban-model').value:'';
    if(!title) return;
    fetch(LaRuche.API.base+'/api/kanban',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title: title, description: desc, profile_id: pId||null, model: m||null})})
  ...
  function saveKanbanEdit(id, btn) {
    ...
    fetch(LaRuche.API.base+'/api/kanban/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({title: title, description: desc, profile_id: pId||null, model: m||null})})
  ...
  async function loadOnboarding(el) {
    ...
    (data.steps||[]).forEach(function(s){
      var icon = s.done ? '<span style="color:var(--green);font-size:18px;margin-right:8px">&#x2713;</span>' : '<span style="color:var(--red);font-size:18px;margin-right:8px">&#x2717;</span>';
  ...
  function exportOkf() {
    fetch(LaRuche.API.base+'/api/memory/export_okf?dir=okf-export')
      .then(function(r){return r.json();})
      .then(function(res){
        if(res.ok) LaRuche.Toast.show('OKF exporte dans ' + res.dir, 'ok');
        else LaRuche.Toast.show('Erreur export: ' + res.error, 'err');
      });
  }
  ...
  return { init:init, openBlueprintForm:openBlueprintForm, instanciateBlueprint:instanciateBlueprint, enter:enter, leave:leave, createCron:createCron, createWatcher:createWatcher, refreshTab:refreshTab, saveChannels:saveChannels, saveContextCfg:saveContextCfg, saveProviderCfg:saveProviderCfg, addKnowledge:addKnowledge, exportOkf:exportOkf, importOkf:importOkf, deleteKnowledge:deleteKnowledge, editKnowledge:editKnowledge, saveKnowledgeEdit:saveKnowledgeEdit, startChannel:startChannel, stopChannel:stopChannel, showProfileForm:showProfileForm, editProfile:editProfile, deleteProfile:deleteProfile, saveProfile:saveProfile, onProfileProviderChange:onProfileProviderChange, startCodexLogin:startCodexLogin, logoutCodex:logoutCodex, toggleTool:toggleTool, toggleAllTools:toggleAllTools, loadSkills:loadSkills, toggleSkill:toggleSkill, deleteSkill:deleteSkill, newSkill:newSkill, viewSkill:viewSkill, saveSkill:saveSkill, createKanbanTask:createKanbanTask, loadMcpServers: loadMcpServers, createMcpServer: createMcpServer, deleteMcpServer: deleteMcpServer, updateKanbanModelSelect: updateKanbanModelSelect, updateWatcherModelSelect: updateWatcherModelSelect, deleteKanbanTask:deleteKanbanTask, editKanbanTask:editKanbanTask, saveKanbanEdit:saveKanbanEdit, kanbanDragStart:kanbanDragStart, kanbanDragOver:kanbanDragOver, kanbanDrop:kanbanDrop, addCredential:addCredential, deleteCredential:deleteCredential, updateCronModelSelect:updateCronModelSelect, updateCronEditModelSelect:updateCronEditModelSelect, toggleVisibility:toggleVisibility, tlZoom:tlZoom, tlRecenter:tlRecenter, tlDetail:tlDetail, tlReload:tlReload, tlRun:tlRun, tlEdit:tlEdit, tlSaveEdit:tlSaveEdit, tlToggle:tlToggle };
})();

/* ── Register all page modules ── */
LaRuche.Router.register('chat', LaRuche.Chat);
LaRuche.Router.register('dashboard', LaRuche.Dashboard);
LaRuche.Router.register('sessions', LaRuche.SessionsPage);
LaRuche.Router.register('memory', LaRuche.Memory);
LaRuche.Router.register('settings', LaRuche.Settings);
LaRuche.Router.register('console', LaRuche.Console);
