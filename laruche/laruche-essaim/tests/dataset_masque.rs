...
fn le_masquage_couvre_les_champs_du_dataset() {
    ...
    assert!(
        !masque.contains("sk-proj-abcdef1234567890"),
        "la cle a fuite: {masque}"
    );
    assert!(
        masque.contains("[SECRET:OPENAI_KEY]"),
        "pas remplace: {masque}"
    );
    ...
    assert_eq!(
        laruche_essaim::secrets::substituer("${OPENAI_KEY}"),
        "sk-proj-abcdef1234567890"
    );
    assert_eq!(
        laruche_essaim::secrets::substituer("${ABSENT}"),
        "${ABSENT}"
    );
}
