...
impl Abeille for GitStatus {
    ...
    fn description(&self) -> &str {
        "Show the git status of the current working directory (modified, staged, untracked files)."
    }
...
impl Abeille for GitDiff {
    ...
    fn description(&self) -> &str {
        "Show the git diff (changes not yet staged). Optionally specify a file path."
    }
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
    ...
    async fn executer(
        ...
        // First get summary
        let summary = run_git(&git_args, &ctx.working_dir)
            .await
            .unwrap_or_default();
        ...
        let diff = run_git(&diff_args, &ctx.working_dir)
            .await
            .unwrap_or_default();
...
impl Abeille for GitLog {
    ...
    fn description(&self) -> &str {
        "Show recent git commits. Default: last 10 commits."
    }
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
    ...
    async fn executer(
        ...
        match run_git(
            &["log", "--oneline", "--graph", &format!("-{}", count)],
            &ctx.working_dir,
        )
        .await
        {
...
impl Abeille for GitCommit {
    ...
    async fn executer(
        ...
        match run_git(&["commit", "-m", message], &ctx.working_dir).await {
            Ok(out) => Ok(ResultatAbeille::ok(format!(
                "Committed: {}\n{}",
                message, out
            ))),
