...
pub fn construire_prompt_resume(output: &str) -> String {
    ...
    format!(
        "Resume ce resultat d'outil en francais, en conservant les faits, chemins, erreurs et prochaines actions utiles.\n\nResultat:\n{preview}\n\nResume:"
    )
}

pub fn resume_extractif(output: &str) -> String {
    format!("[Resume extractif]\n{}", head_tail(output, 1_200))
}
...
mod tests {
    ...
    fn resume_vide_retombe_sur_extractif() {
        ...
        assert!(summary.summarized);
        assert!(summary.text.contains("Resume extractif"));
    }
}
