...
use laruche_essaim::{
    abeilles::enregistrer_abeilles_builtin, brain::boucle_react, AbeilleRegistry, ChatEvent,
    EssaimConfig, Session,
};
...
async fn main() -> Result<()> {
    ...
    if let Some(ref dir) = cwd {
        if dir.exists() {
            ...
            eprintln!("{} Not found: {}", "ERROR".red().bold(), dir.display());
            std::process::exit(1);
        }
    }
    match command {
        ...
        "auth" => cmd_auth(&prompt_args).await?,
        "mcp" => cmd_mcp().await?,
...
fn print_banner() {
    ...
    eprintln!(
        "  {}",
        r"  _            ____           _          ".with(AMBER)
    );
    eprintln!(
        "  {}",
        r" | |    __ _  |  _ \ _   _ __| |__   ___ ".with(AMBER)
    );
    eprintln!(
        "  {}",
        r" | |   / _` | | |_) | | | / _| '_ \ / _ \".with(AMBER)
    );
    eprintln!(
        "  {}",
        r" | |__| (_| | |  _ <| |_| \__|  | ||  __/".with(AMBER)
    );
    eprintln!(
        "  {}",
        r" |_____\__,_| |_| \_\\__,_|___|_| |_|\___|".with(AMBER)
    );
    eprintln!();
    eprintln!(
        "  {} {} {}",
        "L'Essaim Agent".bold(),
        format!("v{}", VERSION).dark_grey(),
    ...
    eprintln!(
        "  {}  {}   {}  {}   {}  {}",
        "cwd".dark_grey(),
        cwd.display().to_string().with(AMBER),
        "model".dark_grey(),
...
fn get_ollama_url() -> String {
    std::env::var("OLLAMA_URL")
        .or_else(|_| std::env::var("LARUCHE_OLLAMA_URL"))
...
async fn pick_model(current: &str) -> Option<String> {
    ...
    for (i, name) in models.iter().enumerate() {
        ...
        eprintln!(
            "  {} {} {}",
            format!("{:>2}", i + 1).with(AMBER),
            cur,
            name.as_str().with(Color::Cyan)
        );
    }
    eprintln!("  {}", "─".repeat(40).dark_grey());
            ...
            match cmd {
                ...
                "/export" | "/e" => {
                    ...
                    for msg in &session.messages {
                        match msg {
                            laruche_essaim::Message::User(t) => {
                                md.push_str(&format!("**User:** {}\n\n", t))
                            }
                            laruche_essaim::Message::Assistant(t) => {
                                md.push_str(&format!("{}\n\n---\n\n", t))
                            }
                            _ => {}
                        }
        ...
        match result {
            Ok(_) => {
                ...
                eprintln!(
                    "  {} ~{} tokens {} {}",
                    "─".repeat(35).dark_grey(),
                    session.estimated_tokens().to_string().dark_grey(),
                    "model:".dark_grey(),
                    current_model.borrow().as_str().dark_grey()
                );
                eprintln!();
            }
            Err(e) => {
    ...
    match provider {
        "codex" | "openai-codex" | "chatgpt" => {
            if flag == "--status" || flag == "status" {
                return auth_codex_status().await;
            }
...
async fn cmd_server(args: &[String]) -> Result<()> {
    ...
    match sub {
        ...
        _ => {
            eprintln!(
                "\n  {} {}\n",
                "LaRuche Server".with(AMBER).bold(),
                "Management".dark_grey()
            );
            eprintln!("  {}:", "Commands".bold());
            eprintln!(
                "    {} {}    {}",
...
async fn server_start() -> Result<()> {
    ...
    if exe.is_none() {
        eprintln!(
            "  {} Server binary not found. Run: laruche server install",
            "✗".red()
        );
        return Ok(());
    }
...
async fn server_install() -> Result<()> {
    ...
    if source_dir.is_none() {
        ...
        eprintln!(
            "  {} Clone the repo: git clone https://github.com/infinition/LaRuche",
            "→".dark_grey()
        );
        return Ok(());
    }
    ...
    match build {
        Ok(s) if s.success() => {
            ...
            if cfg!(windows) {
                eprintln!(
                    "\n  {} To start at boot (run as Admin):",
                    "Info".with(AMBER)
                );
                eprintln!(
                    "    sc.exe create LaRuche binPath= \"{}\" start= auto",
...
async fn server_uninstall() -> Result<()> {
    ...
    if cfg!(windows) {
        ...
        let _ = std::process::Command::new("systemctl")
            .args(["disable", "laruche"])
            .output();
        let _ = std::process::Command::new("rm")
            .args(["-f", "/etc/systemd/system/laruche.service"])
            .output();
        let _ = std::process::Command::new("systemctl")
            .args(["daemon-reload"])
            .output();
...
async fn server_logs() -> Result<()> {
    if cfg!(windows) {
        eprintln!(
            "  {} Windows: check the terminal where laruche-node was started",
            "Info".with(AMBER)
        );
        eprintln!(
            "  {} Or run: Get-EventLog -LogName Application -Source LaRuche",
            "→".dark_grey()
        );
    } else {
...
pub async fn probe_running() -> bool {
    let port = std::env::var("LARUCHE_PORT")
        .ok()
