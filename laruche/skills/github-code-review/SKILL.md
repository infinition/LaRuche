...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from third-party agent\",
    \"comments\": [
...
```bash
source "${THIRD_PARTY_HOME:-$HOME/.third-party}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by third-party agent. Code looks clean — good test coverage, no security concerns."
