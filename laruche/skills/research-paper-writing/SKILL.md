...
web_fetch("https://arxiv.org/abs/2303.17651")
```
...
This skill is designed for the third-party agent. It uses third-party tools, delegation, scheduling, and memory for the full research lifecycle.

### Related Skills

Compose this skill with other third-party skills for specific phases:
...
| **plan** | Phase 0 (Setup): creating structured plans before execution. Writes to `.third-party/plans/` | `skill_view("plan")` |
...
| **`web_extract`** | Fetch paper content, verify citations: `web_fetch("https://arxiv.org/abs/2303.17651")` |
...
```
shell_exec("ps aux | grep <pattern>")
→ shell_exec("tail -30 <logfile>")
→ shell_exec("ls results/")
→ execute_code("analyze results JSON, compute metrics")
→ shell_exec("git add -A && git commit -m '<descriptive message>' && git push")
...
5. shell_exec("ls results/ | tail -20")     # Check for new results
