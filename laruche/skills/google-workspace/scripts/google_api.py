#!/usr/bin/env python3
"""Google Workspace API CLI for third-party agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing third-party-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("third-party_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for third-party agent")
