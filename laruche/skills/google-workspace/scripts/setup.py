#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for third-party agent.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same third-party profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the third-party Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the third-party Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'third-party agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to third-party home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for third-party")
