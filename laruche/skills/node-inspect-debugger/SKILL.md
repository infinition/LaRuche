...
metadata:
  third-party:
    ...
    related_skills: [systematic-debugging, python-debugpy, debugging-third-party-tui-commands]
---
...
third-party-specific note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:
...
```bash
cd /home/bb/third-party agent/ui-tui
...
### Debugging a running `third-party --tui`
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In third-party, launch it with `shell_exec(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
