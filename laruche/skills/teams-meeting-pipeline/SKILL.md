...
description: "Operate the Teams meeting summary pipeline via third-party CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Everything operator-facing is a `third-party teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${THIRD_PARTY_HOME:-~/.third-party}/.env`:
...
```bash
third-party teams-pipeline validate              # config snapshot — run first after any change
third-party teams-pipeline token-health          # Graph token status
third-party teams-pipeline token-health --force-refresh   # force a fresh token acquisition
third-party teams-pipeline list                  # recent meeting jobs
third-party teams-pipeline list --status failed  # only failed jobs
third-party teams-pipeline show <job-id>         # full detail of one job
third-party teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
third-party teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
third-party teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
third-party teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
third-party teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

third-party teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
third-party teams-pipeline delete-subscription <sub-id>
third-party teams-pipeline maintain-subscriptions            # renew near-expiry ones
third-party teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `third-party teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `third-party cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
