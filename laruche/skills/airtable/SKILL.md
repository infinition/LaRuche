...
4. Store the token in `${THIRD_PARTY_HOME:-~/.third-party}/.env` (or via `third-party setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for third-party. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for third-party
...
- **`AIRTABLE_API_KEY` flows from `${THIRD_PARTY_HOME:-~/.third-party}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
