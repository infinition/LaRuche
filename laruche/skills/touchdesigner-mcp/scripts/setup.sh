...
# ── 3. Ensure third-party config has twozero_td MCP entry ──
if [[ ! -f "$third-party_CFG" ]]; then
    echo -e " ${FAIL} third-party config not found at ${third-party_CFG}"
    manual_steps+=("Create ${third-party_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$third-party_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in third-party config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to third-party config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$third-party_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${third-party_CFG} manually"); }
    manual_steps+=("Restart third-party session to pick up config change")
fi
