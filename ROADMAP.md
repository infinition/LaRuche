...
- [x] **`brain.rs` dissous en modules (2026-07-02)** : déplacement pur (comportement identique, zéro changement de logique), un module = un commit vert. `config.rs` (227 l. : `EssaimConfig`, `ReineConfig`) · `evenements.rs` (130 l. : `ChatEvent`, `PlanItem`, `ApprovalResponse`/`Receiver`, `SteerReceiver`) · `parsing.rs` (299 l. : `parse_tool_calls*`, `parse_plan`, `ToolCall`) · `permissions.rs` (312 l. : `decision_permission`, `garde_injection`, `timeout_for_tool`) · `contexte.rs` (1523 l. : sélection dynamique des outils, catalogue de capacités, wrapper `boucle_react_memoire*` pré-retrieval/working-set/index skills, façade `boucle_react_multimodal_ext` -> `butinage_pont`) · `curation.rs` (527 l. : extraction de faits durables, détection de contradictions, consolidation mémoire, apprentissage de skills OKF). `brain.rs` passe de 3005 à 26 lignes : il ne reste qu'un shim `pub use` sous le chemin historique `brain::`, gardé exprès (pas supprimé) parce que 20 sites dans `laruche-node`/`laruche-cli`/`laruche-evals` importent via `laruche_essaim::brain::X` et que la mission bornait le diff à `laruche-essaim` seul. `cargo test --workspace --exclude laruche-evals` : 339 verts (0 échec), zéro warning nouveau (seuls les 4 warnings préexistants de `laruche-cli` subsistent). Migration `session.rs` (`laruche_compaction`, type `session::Message`) vers `but::escale` (opère sur `Carnet`/`messagerie::Message` de butinage, état de mission crash-resumable) **étudiée et écartée** : modèles de données incompatibles (types Message distincts, `Session` consommé tel quel par node/cli/evals) - migrer forcerait soit une couche de mapping avec perte, soit remplacer `Session` partout hors `laruche-essaim`. Hors scope d'un refacto pur ; à reprendre en mission dédiée si besoin.
...
- [~] **OKF + git** : ✅ **time-travel livré (2026-07-02 soir)** - dépôt git dédié `memoire-okf/` (snapshot OKF toutes les 30 min si changement, timestamps neutralisés, `LARUCHE_OKF_GIT_SECS`), `git log/diff` = chronologie et diffs de la mémoire, rollback ciblé via checkout + import. Reste : **fédération mesh des faits** entre nœuds (provenance) - 2 nœuds requis.
...
- [x] **Watchers v3 : règles compilées (2026-07-02)** : DSL de prédicats déterministes (`et/ou/non`, jours/heures/dates, apparu/supprimé/modifié, down_depuis_min, contient, taille, status_http) évalué gratuitement à chaque poll ; `llm_check` = seule feuille coûteuse, en court-circuit après le préfixe déterministe. Skill `watcher-architecte` (compilation par l'agent), outil/API/dispatcher/UI (résumé auditable + JSON éditable dans la bulle). Sustained intégré (règle d'état + cooldown). Chemin legacy conservé.
...
- [x] **Localisation EN** : prompts système, abeilles (318 chaînes), plugins, **71 skills** (lean + `tools:` liés).
...
- [x] **Sentinels Rust↔JS** traduits **conjointement** : verbes de feed (`a demandé/a répondu`→`asked/replied` + matcher capabilities.js), `Synthèse/Erreur LaRuche`→`LaRuche summary/error` + matcher memory.js, `réponse finale`→`final response`, séparateur de description `" - "` (au lieu de l'em dash).
